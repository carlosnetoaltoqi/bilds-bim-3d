...
export default async function HomePage() {
  ...
  try {
    const res = await fetch(`${CATALOGO_URL}/empresas`, { cache: 'no-store' })
    if (!res.ok) throw new Error(`API ${res.status}`)
    const lista = (await res.json()) as Empresa[]
    empresas = await Promise.all(lista.map(async (e) => {
      const r = await fetch(`${CATALOGO_URL}/empresas/${e.customUrl}/catalogos`, { cache: 'no-store' })
      return { ...e, catalogos: r.ok ? ((await r.json()) as Catalogo[]) : [] }
    }))
  } catch (e: any) {
    erro = `não consegui falar com a API em ${CATALOGO_URL} — ${e?.message ?? e}`
  }
        ...
        {empresas.map((e) => (
          <section key={e.id} className="bg-white border border-gray-200 rounded-lg mb-5">
            <header className="flex items-center gap-3 px-5 py-3 border-b border-gray-100">
              {e.logoUrl ? <img src={`${CATALOGO_URL}${e.logoUrl}`} alt="" className="w-9 h-9 object-contain rounded border border-gray-200" /> : <span className="w-9 h-9 rounded bg-gray-100" />}
