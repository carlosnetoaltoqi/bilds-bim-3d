...
import { catalogoJson } from '@/servicos/catalogo'
import { editorJson } from '@/servicos/editor'
...
export function InfoForm({ produto, series, onSaved }: InfoFormProps) {
  ...
  async function salvar() {
    ...
    try {
      ...
      const salvo = await editorJson<ProdutoDto>(`/produtos/${produto._id}`, { method: 'PATCH', body: JSON.stringify(body) })
      onSaved(salvo)
...
export function CatalogForm({ catalog, onSaved }: { catalog: PocCatalog; onSaved: (c: PocCatalog) => void }) {
  ...
  async function salvar() {
    ...
    setMsg(null)
    try {
      const c = await catalogoJson<PocCatalog>(`/catalogos/${catalog.id}`, { method: 'PATCH', body: JSON.stringify({ title, manufacturer, layout }) })
      onSaved(c)
