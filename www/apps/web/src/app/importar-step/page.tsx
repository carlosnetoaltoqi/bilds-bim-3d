 ...
 * (lido pelo parse_ifc.py do projeto); a API cria um produto num catálogo e o
 * editor 3D abre a peça como qualquer outra.
...
export default function ImportarStepPage() {
  ...
  async function enviar(e: FormEvent) {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/cad/importar`, { method: 'POST', body: fd })
    ...
    <main className="min-h-screen bg-gray-50 text-gray-900 flex items-start justify-center py-12 px-6" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div className="w-full max-w-[560px]">
        ...
        <p className="text-[13px] text-gray-600 mb-6">
          STEP é tesselado no servidor com OpenCASCADE; IFC é lido pelo <code>parse_ifc.py</code> do projeto (placements, instâncias, cores por face).
          Os dois viram metros, Y-up, e entram num catálogo como produto. Depois é só editar, e exportar em IFC4 ou <code>.aq</code>.
        </p>
        ...
        {res && (
          <div className="mt-5 bg-white border border-green-200 rounded-lg p-5 text-[13px]">
            ...
            <p className="text-gray-700 mb-3">
              {res.formato === 'ifc' ? 'IFC' : 'STEP'} · {res.partes} {res.formato === 'ifc' ? 'produto(s)' : 'sólido(s)'} · {res.triangulos.toLocaleString('pt-BR')} triângulos · {res.bbox_mm.map((v) => v.toFixed(1)).join(' × ')} mm · unidade do arquivo {res.unidade}
            </p>
