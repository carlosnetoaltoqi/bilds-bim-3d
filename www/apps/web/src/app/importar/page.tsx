...
function ImportarPageInner() {
  ...
  async function carregarUltimas(emp: string) {
    try {
      const r = await fetch(`${INGESTAO_URL}/importacoes?limite=8${emp ? `&empresa=${encodeURIComponent(emp)}` : ''}`, { cache: 'no-store' })
      if (r.ok) {
  ...
  useEffect(() => {
    ...
    timer.current = setInterval(async () => {
      try {
        const r = await fetch(`${INGESTAO_URL}/importacoes/${atual.importId}`, { cache: 'no-store' })
  ...
  function enviar(e: FormEvent) {
    ...
    xhr.open('POST', `${INGESTAO_URL}/importacoes`)
    xhr.upload.onprogress = (ev) => { if (ev.lengthComputable) setPct(Math.round((ev.loaded / ev.total) * 100)) }
    xhr.onerror = () => { setErro(`falha de rede ao enviar — o serviço de ingestão está de pé em ${INGESTAO_URL}?`); setEnviando(false); setPct(null) }
    xhr.onload = () => {
