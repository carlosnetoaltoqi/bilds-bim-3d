...
export function ProductEditor(props: Props) {
  ...
  const geoUrl = `${API_URL}/geometrias/${produto._id}`
  ...
  useEffect(() => {
    ...
    fetch(`${geoUrl}/original?t=${Date.now()}`, { cache: 'no-store' })
  ...
  /** Tessela um STEP no servidor e acrescenta as partes ao modelo atual. */
  async function importStep(file: File) {
    if (!hist) return
    setSaving(true)
    setMsg({ tipo: 'info', texto: `tesselando ${file.name}…` })
    try {
      const fd = new FormData()
      fd.append('file', file)
      const r = await fetch(`${API_URL}/step/tesselar`, { method: 'POST', body: fd })
      const data = await r.json().catch(() => ({}))
      if (!r.ok) throw new Error(data?.message ? String(data.message) : `API ${r.status}`)
      const novas = segment(data as GeoData).map((p) => ({ ...p, nome: `${file.name.replace(/\.(stp|step)$/i, '')} · ${p.nome}` }))
      ...
      setMsg({ tipo: 'erro', texto: `STEP: ${e.message ?? String(e)}` })
      throw e
    } finally {
      setSaving(false)
    }
  ...
  async function restore() {
    ...
    setSaving(true)
    try {
      await apiJson(`/geometrias/${produto._id}/restaurar`, { method: 'POST' })
      invalidateGeo(geoUrl)
  ...
  return (
    <div className="h-screen flex flex-col bg-white text-gray-900" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <header className="flex items-center gap-3 px-4 h-12 border-b border-gray-200 bg-[#002D72] text-white shrink-0">
        ...
        <a href="/importar-step" className="text-[12px] text-blue-200 hover:text-white">importar STEP</a>
