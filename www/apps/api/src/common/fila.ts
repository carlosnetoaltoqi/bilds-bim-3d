 ...
 *
 * As importações (`.aq` pelo parse-worker; STEP/IFC pelo Python) rodavam todas ao mesmo
 * tempo: dois uploads = dois workers + dois Chromiums + duas conversões Python disputando
 * CPU e RAM, e nada dizia ao usuário que ele estava esperando. Agora os dois serviços
 * passam o processamento por UMA instância desta fila (`FILA_IMPORTACOES`), concorrência
 * 1 por padrão (`IMPORTACOES_CONCORRENCIA` no env).
 *
 * É só em memória — a fila morre com o processo. O que estava na fila ou em execução
 * quando a API caiu é tratado pelo `RecuperacaoService` no boot (marca `falhou`).
