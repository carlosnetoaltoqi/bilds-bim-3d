/**
 * thumb-rasterizer-sw.ts — rasterizador software (Abordagem B do ADR-003)
 *
 * HISTÓRICO. Não é mais o caminho de produção: desde 2026-08-30 as miniaturas do
 * POC saem do `thumb-rasterizer.ts` (Playwright + harness.html + Three.js real),
 * porque o resultado deste arquivo diverge visivelmente do viewer — ver
 * `docs/solutions/architecture-patterns/thumb-qualidade-identica-ao-viewer.md`.
 *
 * Continua aqui só para `tools/measure-thumbs.ts` conseguir remedir a comparação
 * A × B que originou o ADR-003. Nenhum código de runtime o importa.
 *
 ...
 *   - Sombreamento: flat (normal por triângulo), não smooth
 *   - Anti-aliasing: SSAA box-average 2×, não MSAA
 ...
 */
export async function renderThumbSw(
  data: RasterBuffers,
  width = THUMB_W,
  height = THUMB_H,
): Promise<Buffer> {
