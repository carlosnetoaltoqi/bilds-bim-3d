...
| **ADR-003** | **Abordagem de geração de miniaturas no servidor.** Medido em S2.4 com 39 produtos Dancor (geometrias reais do Atlas). **Abordagem A — Playwright + Chromium + SwiftShader:** 240 ms/geo, 5,5 KB/WebP, +startup de ~2-5 s, imagem Docker ~1,5 GB, PBR idêntico ao viewer. **Abordagem B — rasterizador TS + ffmpeg:** 65 ms/geo, 4,3 KB/WebP, sem browser, sem startup; sombreamento plano (ambient + 2 luzes direcionais, z-buffer, projeção perspectiva). **Decisão: Abordagem B.** B é 3,7× mais rápido por geometria, imagens menores, sem dependência de Chromium no pod. A diferença visual (flat shading vs. PBR) é aceitável para miniaturas de catálogo. A abordagem A vence em fidelidade, perde em operação. Não reabrir sem mudança no requisito de fidelidade. ⚠️ **REVERTIDO em S4.4 (2026-08-30): vale a Abordagem A.** O requisito de fidelidade mudou — o dono do projeto avaliou o resultado da B e o recusou. Medido contra o render do viewer: **B = 27 dB de PSNR, A = 47 dB** (o piso da compressão WebP q=0,85). O custo de operação da A revelou-se menor do que a S2.4 estimou: com o browser reaproveitado por processo e a geometria passada como string JSON (e não como objeto) para o `page.evaluate`, são **~370 ms por miniatura**, não 240 ms + startup por thumb — 13 produtos em ~6,3 s. Ver ADR-004. | **reaberta e revertida** | S2.4 (2026-08-30) → **S4.4 (2026-08-30)** |
| **ADR-004** | **Miniaturas do servidor saem do mesmo Three.js do viewer.** Substitui o ADR-003. O `thumb-worker.ts` renderiza via Playwright + `templates/thumbs/harness.html` — o mesmo harness, o mesmo `buildScene()` e a mesma câmera do pipeline estático e do viewer. **Decisão: qualquer reimplementação do render fora do Three.js é proibida**; a divergência visual não é negociável para o card de catálogo. Custo operacional aceito: ~200 MB de Chromium por processo worker e a exigência das libs de sistema (`libnss3 libnspr4 libasound2t64`) na imagem. Duas regras de implementação, sem as quais a decisão não se sustenta: **(a)** um browser por processo, fechado explicitamente antes do `process.exit()`; **(b)** geometria vai ao `page.evaluate` como **string JSON**, nunca como objeto — 6× de diferença. | **fechada** | S4.4 (2026-08-30) |
...
| S4.4 | **concluída** | 2026-08-30 | [S4.4](sessoes/S4.4-thumbs-playwright.md) | miniaturas passam a sair do Three.js real (Playwright + harness); PSNR contra o viewer 27 dB → 47 dB; 13 produtos em ~6,3 s; ADR-003 revertido, ADR-004 aberto e fechado |
