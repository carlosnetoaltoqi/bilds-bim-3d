...
- **S7.18:** `pipeline/zip_bilds.py::build_zip_bilds` duplica `scripts/build.py::build_zip` (manifesto, dedupe de geometria, thumbs). Pela direção da S7.14 o `build.py` deve importar o do pipeline (mantendo o nome `<slug>-<ts>.zip` e os `AVISO` de geometria ausente) — um commit com teste. E `tests/test_zip_bilds.py::test_cli_akato` aponta para `input/Akato/Akato.aq`, que não existe: usar `conftest.AKATO_AQ` — hoje o teste pula sempre.
