...
| `python3 -m bim_pipeline.cli.ferramentas.preencher_entradas_aq <arquivo.aq> [--saida copia.aq] [--refazer]` | Acha os bocais no OQ3D que já está no arquivo e grava `ENTRADA_3D`/`ENTRADA_PECA`, marca `CONEXAO_VOLUMETRICA` e anula a seção da peça. Varre também `.aq` que **já** tinha entradas, que é o caso das bibliotecas exportadas em 2026-09-09 |
| `python3 -m bim_pipeline.cli.ferramentas.ajuda_builder [--ajuda <dir> --indexar] <regex>` | Busca na ajuda em HTML do Builder — a única fonte que diz **o que um campo significa**. Indexa ~2.500 páginas uma vez num JSONL e busca sobre o texto, **pulando as 281 páginas órfãs de versão antiga** (`--incluir-orfas` traz de volta); o diretório da ajuda é uma instalação do Builder, não do repositório |
