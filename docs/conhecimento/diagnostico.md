...
| Dois Chromiums ao mesmo tempo depois de dois uploads (`thumb-worker iniciado` de dois imports com 1 s de diferença no log) | Antes da S7.13 a fila liberava a vaga em `publicado`; hoje só depois das miniaturas — o segundo fica em `recebido` até o Chromium do primeiro fechar (Amanco: ~80 s). `IMPORTACOES_CONCORRENCIA` > 1 é a única forma legítima de ver dois |
| `geo/<importId>/` ganhando arquivos **depois** de a API morrer; `pgrep -af parse-worker` mostra um filho sem pai | parse-worker órfão — corrigido na S7.13 (os workers saem com 2 no `disconnect`). Se reaparecer, alguém tirou o handler ou o filho foi criado sem canal IPC (`fork` sem `silent`/IPC) |
| Nome de arquivo com `Ã¡`, `Ã©`, `Ã§` no log `upload recebido`, no `fileName` do import ou no nome do produto CAD | O multer que roda é o 2.0.2 embutido no `@nestjs/platform-express` 10 (decodifica latin1; ignora `defParamCharset`). `common/upload.ts` refaz em UTF-8 — se apareceu, um caminho novo usa `originalname` direto; `tests/test_www_config.py` acusa |
...
| `GET /catalogos/…` (ou qualquer rota com Mongo) demora 30 s e devolve `500 Internal server error` | Mongo inacessível (Atlas bloqueado, whitelist, rede): o driver espera `serverSelectionTimeoutMS`. `GET /health` responde **503 na hora** — olhe-o primeiro. I32 na auditoria |
...
| `kill -9` na API e ela continua respondendo; ou `pkill -f` derrubou o seu próprio terminal | `pnpm dev:api` cria `sh -c` → `node`: `pgrep -f 'src/main.ts' \| head -1` pega o `sh`. Mate o pid de `ss -ltnp \| grep ':4000 '` (e o pai por `ps -o ppid=`). `pkill -f '<padrão>'` casa com a linha de comando do shell que o executa se o padrão estiver nela |
