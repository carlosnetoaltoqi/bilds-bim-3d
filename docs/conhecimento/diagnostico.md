...
| Sintoma | Causa | O que fazer |
|---|---|---|
| **Peça desenha em 3D mas em PLANTA sai como um círculo com um triângulo vermelho** | É o símbolo padrão do Builder para peça sem representação 2D: falta a simbologia 2D **ou** o `WIREFRAME`, e o `WIREFRAME` o Builder gera sozinho — desde que a peça tenha pontos de ligação 3D e "Bifiliar realista" não esteja em "Simbologia 2D" | Escrever `ENTRADA_3D`/`ENTRADA_PECA` (já ligadas aos dois escritores) ou, num `.aq` já exportado, rodar `ferramentas.preencher_entradas_aq`. Conferir no Cadastro que "Pontos de ligação 3D" ficou "Sim" |
| Simbologia sem bocal reconhecível ao preencher as entradas | A malha não tem face anelar de ponta de tubo — sólido maciço, malha aberta, ou peça que não é hidráulica (ponto de ligação de rack é entrada de cabo, não abertura) | Esperado: o detector não inventa ponto de ligação. A contagem sai no fim da ferramenta; `bocais.bocais(malhas)` aceita `raio_min`/`raio_max` para casos fora da faixa de 0,3 a 25 cm |
| Simbologia descartada por ter "bocais fora do que uma peça tem" | Mais de 38 bocais (o máximo de uma simbologia nativa) — típico de projeto inteiro importado como uma peça só | Esperado: é sinal de que a fonte não é uma peça. Se a peça for legítima e enorme, `entradas_aq.plausivel(entradas, limite=N)` aceita outro limite |
| Bocal achado mas `DIAMETRO_EP` na sentinela | A bitola não cai na escala de diâmetros do AltoQi (40 a 200 mm, passos de PVC) — típico de conexão *press* de aço, onde o encaixe tem o diâmetro **externo** do tubo | Esperado; a posição da entrada está certa e é o que destrava planta e encaixe. Preencher o código exige a escala completa do AltoQi para o sistema em questão |
