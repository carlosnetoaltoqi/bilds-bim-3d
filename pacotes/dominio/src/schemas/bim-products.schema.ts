...
export class BimProduct {
  ...
  /**
   * Chave da miniatura no GeometryStore. Desde a E3 a miniatura é renderizada por GEOMETRIA
   * (`thumbs/<importId>/<stem do geoKey>.webp`), então produtos que compartilham `geoKey`
   * compartilham também a miniatura.
   */
  @Prop()
  thumbKey: string;

  /**
   * Copy-on-write (A5 de docs/arquitetura-www-servico-de-ingestao.md): o pipeline grava UMA
   * geometria por simbologia e vários produtos apontam para ela. Na primeira edição de um
   * produto cuja geometria é compartilhada, a API grava um arquivo só dele
   * (`geo/<importId>/<productId>.json`) em `geoKey` e guarda aqui a chave compartilhada —
   * é o "original" para `GET /geometrias/:id/original` e para `restaurar`. Null quando a
   * geometria é exclusiva (aí o original é o `.orig.json` ao lado).
   */
  @Prop({ type: String })
  geoKeyCompartilhada: string | null;
