...
import { BimCatalog, BimCatalogDocument, BimImport, BimImportDocument, BimProduct, BimProductDocument, Company, CompanyDocument, IGeometryStore, NaoEncontrado, apagarEmpresa } from '@bim/dominio';
...
export class EmpresasController {
  ...
  async create(
    ...
    if (logo) {
      ...
      await this.store.put(logoKey, logo.buffer);   // pelo IGeometryStore, como toda escrita no storage (S8/F4)
    }
  ...
  async logo(@Param('companyId') companyId: string, @Res() res: Response) {
    ...
    let blob: Buffer;
    try {
      blob = await this.store.get(company.logoKey);
    } catch (err: any) {
