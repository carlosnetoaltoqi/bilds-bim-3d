...
| `POST /importacoes/familias-revit` (multipart `file` = `.rfa` ou `.zip` de famílias + `empresa?`, `catalogo?`, `fabricante?`, `comprimentoMm?`, `deflexao?`) | famílias Revit → catálogo (`familias_revit importar`): tipos do PartAtom/type catalog; geometria do IFC/STEP/IGES irmão ou forma representativa por parâmetro (`docs/conhecimento/revit-familias.md`). Inspeção síncrona recusa na hora o que não tem família (um `.rvt`, por exemplo). Um `.rfa` solto em `POST /importacoes` cai aqui |
